...
# MAGIC  
# MAGIC **Data input**  
# MAGIC * **Cohort: `.ccu013_covid_events`**  
# MAGIC * Key demographics: `.curr302_patient_skinny_record`  
# MAGIC * LSOA, comorbidity code search: `.ccu003_direfcts_dataprep_1_gdppr_frzon28may_mm_210528`  
...
# MAGIC   
# MAGIC **Data output**  
# MAGIC * `.ccu013_covid_events_demographics` 1 row per patient
# MAGIC 
...
demog_data = spark.table('.curr302_patient_skinny_record') \
...
# Check coverage
events = spark.table('.ccu013_covid_events')
...
SELECT * FROM (SELECT
  ...
  MAX(DATE) as date
FROM
  .ccu003_direfcts_dataprep_1_gdppr_frzon28may_mm_210528
GROUP BY
...
LEFT JOIN
  (SELECT 
    ...
    DATE as date,
    LSOA as lsoa 
  FROM .ccu003_direfcts_dataprep_1_gdppr_frzon28may_mm_210528) as tab2
ON
...
# MAGIC ### 3.2 Long covid
# MAGIC 
# MAGIC Using codelist: `.ccu013_codelist_long_covid`

# COMMAND ----------

codelist_long_covid = spark.table(".ccu013_codelist_long_covid") \
...
FROM 
  .descriptive_stroke_previous_distinct_table
WHERE
  Stroke_previous = 1
  """)
...
FROM 
  .descriptive_mi_previous_distinct_table
WHERE
  MI_previous = 1
  """) 
...
# MAGIC %sql
# MAGIC OPTIMIZE .ccu013_covid_events_demographics ZORDER BY person_id_deid

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT COUNT(*), COUNT(DISTINCT person_id_deid) FROM .ccu013_covid_events_demographics
