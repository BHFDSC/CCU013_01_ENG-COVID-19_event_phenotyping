...
LatestProductionDate = spark.sql("SELECT MAX(ProductionDate) FROM .wrang002b_data_version_batchids").first()[0]
LatestAPC = spark.sql("SELECT MAX(ADMIDATE) FROM .hes_apc_all_years").first()[0]
...
# MAGIC .wrang002b_data_version_batchids
...
# MAGIC %sql
# MAGIC SELECT min(date), max(date) FROM .ccu013_tmp_sgss
...
# GDPPR
gdppr = spark.sql(f'''SELECT DISTINCT NHS_NUMBER_DEID, DATE, CODE, ProductionDate FROM .gdppr__archive 
                     WHERE ProductionDate == "{production_date}"''')
...
# MAGIC %sql
# MAGIC SELECT min(date), max(date) FROM .ccu013_tmp_gdppr
...
# Deaths
death = spark.sql(f'''SELECT * FROM .deaths__archive
                      WHERE ProductionDate == "{production_date}"''')
...
# MAGIC %sql
# MAGIC SELECT min(date), max(death_date) FROM .ccu013_tmp_deaths
...
# MAGIC %sql
# MAGIC SELECT min(date), max(date) FROM .ccu013_tmp_apc
...
# HES CC
cc = spark.sql(f'''SELECT * FROM .hes_cc_all_years_archive
              WHERE ProductionDate == "{production_date}"''')
...
# MAGIC %sql
# MAGIC SELECT min(date), max(date) FROM .ccu013_tmp_cc
...
chess = spark.sql(f'''SELECT PERSON_ID_DEID as person_id_deid, Typeofspecimen, Covid19, AdmittedToICU, Highflownasaloxygen, NoninvasiveMechanicalventilation, Invasivemechanicalventilation,
                     RespiratorySupportECMO, DateAdmittedICU, HospitalAdmissionDate, InfectionSwabDate as date, 'InfectionSwabDate' as date_is FROM .chess__archive WHERE ProductionDate == "{production_date}"''')
...
# MAGIC %sql
# MAGIC SELECT min(date), max(date) FROM .ccu013_tmp_chess
...
# MAGIC %sql
# MAGIC SELECT DISTINCT ProductionDate FROM .sus__archive 
...
DISCHARGE_DESTINATION_HOSPITAL_PROVIDER_SPELL, DISCHARGE_METHOD_HOSPITAL_PROVIDER_SPELL, END_DATE_HOSPITAL_PROVIDER_SPELL FROM .sus_''')
