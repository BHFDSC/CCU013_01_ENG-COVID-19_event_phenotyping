...
# MAGIC inner join .ccu013_tmp_gdppr tab2 on tab1.clinical_code = tab2.code
# MAGIC )
# MAGIC SELECT person_id_deid, date, 
...
# MAGIC FROM .ccu013_tmp_apc as apc
# MAGIC INNER JOIN .ccu013_tmp_cc AS cc
...
# MAGIC %sql
# MAGIC OPTIMIZE .ccu013_covid_severe_death ZORDER BY person_id_deid
...
# MAGIC CREATE TABLE .tmp_ccu013_covid_trajectory_delta USING DELTA
# MAGIC as 
...
# MAGIC %sql
# MAGIC ALTER TABLE .tmp_ccu013_covid_trajectory_delta OWNER TO ;

# COMMAND ----------

# MAGIC %sql
# MAGIC REFRESH TABLE .tmp_ccu013_covid_trajectory_delta
...
for table in ['ccu013_gdppr_covid','ccu013_apc_covid','ccu013_sus_covid','ccu013_chess_covid_hospital','ccu013_chess_covid_icu',
  ...
  (spark.table(f'global_temp.{table}')
   ...
   .saveAsTable('.tmp_ccu013_covid_trajectory_delta'))

# COMMAND ----------

## Add on the fatal covid cases with diagnosis
(spark.table(".ccu013_covid_severe_death").select("person_id_deid", "date", "covid_phenotype", "clinical_code", "description", "covid_status", "code", "source", "date_is")
    .distinct()
    .write.format("delta").mode('append')
    .saveAsTable('.tmp_ccu013_covid_trajectory_delta'))
...
# MAGIC %sql
# MAGIC OPTIMIZE .tmp_ccu013_covid_trajectory_delta ZORDER BY person_id_deid
...
# Get all deaths
all_fatal = spark.sql("""
SELECT * FROM .ccu013_tmp_deaths
""")

# Identify earliest non fatal event from trajectory table
first_non_fatal = spark.sql("""
WITH list_patients_to_omit AS (SELECT person_id_deid FROM .tmp_ccu013_covid_trajectory_delta WHERE covid_phenotype = '04_Fatal_with_covid_diagnosis')
SELECT /*+ BROADCAST(list_patients_to_omit) */
t.person_id_deid, MIN(t.date) AS first_covid_event
FROM .tmp_ccu013_covid_trajectory_delta as t
...
# MAGIC create or replace global temp view ccu013_covid_trajectory AS
# MAGIC select * from .tmp_ccu013_covid_trajectory_delta UNION ALL
...
# MAGIC %sql
# MAGIC OPTIMIZE .ccu013_covid_trajectory ZORDER BY person_id_deid
...
# MAGIC %sql
# MAGIC SELECT covid_phenotype, count(person_id_deid) as count
# MAGIC FROM .ccu013_covid_trajectory
# MAGIC group by covid_phenotype
# MAGIC order by covid_phenotype
...
# MAGIC WITH list_patients_to_omit AS (SELECT person_id_deid from .ccu013_covid_trajectory WHERE covid_phenotype IN ('02_Covid_admission', '03_ECMO_treatment', '03_IMV_treatment', '03_NIV_treatment', '03_ICU_treatment', '04_Fatal_with_covid_diagnosis','04_Fatal_without_covid_diagnosis', '04_Covid_inpatient_death'))
# MAGIC SELECT /*+ BROADCAST(list_patients_to_omit) */ person_id_deid, min(date) as date, '01_not_hospitalised' as covid_severity 
# MAGIC FROM .ccu013_covid_trajectory as t
...
# MAGIC %sql
# MAGIC OPTIMIZE .ccu013_covid_mild ZORDER BY person_id_deid
...
# MAGIC WITH list_patients_to_omit AS (SELECT person_id_deid from .ccu013_covid_trajectory WHERE covid_phenotype IN ('03_ECMO_treatment', '03_IMV_treatment', '03_NIV_treatment', '03_ICU_treatment','04_Fatal_with_covid_diagnosis','04_Fatal_without_covid_diagnosis', '04_Covid_inpatient_death'))
...
# MAGIC %sql
# MAGIC OPTIMIZE .ccu013_covid_moderate ZORDER BY person_id_deid
...
# MAGIC WITH list_patients_to_omit AS (SELECT person_id_deid from .ccu013_covid_trajectory
...
# MAGIC %sql
# MAGIC OPTIMIZE .ccu013_covid_severe ZORDER BY person_id_deid
...
# MAGIC as SELECT * FROM .ccu013_covid_mild
# MAGIC UNION ALL
# MAGIC SELECT * FROM .ccu013_covid_moderate
# MAGIC UNION ALL
# MAGIC SELECT * FROM .ccu013_covid_severe
# MAGIC UNION ALL
...
# MAGIC %sql
# MAGIC OPTIMIZE .ccu013_covid_severity
...
# MAGIC FROM .ccu013_covid_severity
...
# MAGIC -- OLD values @ 04.06.21 = 3977185
# MAGIC -- OLD value pre 15.06.21 = 3772432
# MAGIC -- Current value 15.06.21 3992872
# MAGIC SELECT count(DISTINCT person_id_deid)
# MAGIC FROM .ccu013_covid_severity
