...
# MAGIC %md
# MAGIC **Description** Populates the table `.ccu013_dp_skinny_patient_23_01_2020` with the following for **each patient alive** on **23rd Jan 2020**:
# MAGIC - SEX
# MAGIC - RAW_ETHNICITY
...
# MAGIC 
# MAGIC The records are taken from the `.ccu013_patient_skinny_unassembled` which is made in notebook `ccu013_01_dp_skinny_record_unassebled`. This is simply a list of all the records for each patient from all the datasets.
# MAGIC  
# MAGIC **Project(s)** All
# MAGIC  
...
# MAGIC  
# MAGIC **Date last updated** 2021-06-13
# MAGIC  
# MAGIC **Date last reviewed** 2021-06-13
# MAGIC  
# MAGIC **Date last run** 2020-06-13
# MAGIC  
# MAGIC **Data input** [**table**: `.ccu013_dp_patient_skinny_unassembled`]
# MAGIC 
# MAGIC **Data output** `.ccu013_dp_skinny_patient_23_01_2020`
# MAGIC 
...
# MAGIC %sql
# MAGIC SELECT count(DISTINCT NHS_NUMBER_DEID) from .ccu013_dp_patient_skinny_unassembled
...
FROM .ccu013_dp_patient_skinny_unassembled""")
...
# MAGIC %sql
# MAGIC SELECT count(DISTINCT NHS_NUMBER_DEID) from .ccu013_dp_patient_fields_ranked_pre_cutoff
...
# MAGIC       pres.hes_op,
# MAGIC       pres.hes_ae
# MAGIC FROM (SELECT DISTINCT NHS_NUMBER_DEID FROM .ccu013_dp_patient_fields_ranked_pre_cutoff) pat 
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, ETHNIC FROM .ccu013_dp_patient_fields_ranked_pre_cutoff WHERE ethnic_recency_rank = 1) eth ON pat.NHS_NUMBER_DEID = eth.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, SEX FROM .ccu013_dp_patient_fields_ranked_pre_cutoff WHERE sex_recency_rank = 1) sex ON pat.NHS_NUMBER_DEID = sex.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, DATE_OF_BIRTH FROM .ccu013_dp_patient_fields_ranked_pre_cutoff WHERE birth_recency_rank = 1) dob ON pat.NHS_NUMBER_DEID = dob.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, DATE_OF_DEATH FROM .ccu013_dp_patient_fields_ranked_pre_cutoff WHERE death_recency_rank = 1) dod ON pat.NHS_NUMBER_DEID = dod.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN .ccu013_dp_patient_dataset_presence_lookup pres ON pat.NHS_NUMBER_DEID = pres.NHS_NUMBER_DEID
...
# ? this is more of Sam's approach for building curated tables with demographics and therefore perhaps overkill for our purposes of defining a cohort?
# Or can we just run the definition element on sam's pre-prepared table?
...
# MAGIC %sql
# MAGIC ---- ANNOTATED - leave one out analysis
# MAGIC SELECT count(DISTINCT pat.NHS_NUMBER_DEID) FROM .ccu013_dp_patient_fields_ranked_pre_cutoff pat 
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, ETHNIC FROM .ccu013_dp_patient_fields_ranked_pre_cutoff WHERE ethnic_recency_rank = 1) eth ON pat.NHS_NUMBER_DEID = eth.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, SEX FROM .ccu013_dp_patient_fields_ranked_pre_cutoff WHERE sex_recency_rank = 1) sex ON pat.NHS_NUMBER_DEID = sex.NHS_NUMBER_DEID
# MAGIC         -- Line below Makes no difference to count if removed
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, DATE_OF_BIRTH FROM .ccu013_dp_patient_fields_ranked_pre_cutoff WHERE birth_recency_rank = 1) dob ON pat.NHS_NUMBER_DEID = dob.NHS_NUMBER_DEID
# MAGIC         -- Line below Makes no difference to count if removed
# MAGIC         LEFT JOIN (SELECT NHS_NUMBER_DEID, DATE_OF_DEATH FROM .ccu013_dp_patient_fields_ranked_pre_cutoff WHERE death_recency_rank = 1) dod ON pat.NHS_NUMBER_DEID = dod.NHS_NUMBER_DEID
# MAGIC         LEFT JOIN .ccu013_dp_patient_dataset_presence_lookup pres ON pat.NHS_NUMBER_DEID = pres.NHS_NUMBER_DEID -- Fails if removed
# MAGIC         LEFT JOIN global_temp.ccu013_ethnicity_lookup eth_group ON eth.ETHNIC = eth_group.ETHNICITY_CODE -- Makes no difference to count if removed
# MAGIC WHERE pres.gdppr = 1
...
# MAGIC %sql
# MAGIC SELECT count(DISTINCT NHS_NUMBER_DEID) FROM 
# MAGIC .ccu013_dp_patient_dataset_presence_lookup
# MAGIC where gdppr = 1
...
# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_dp_patient_dataset_presence_lookup tab1
# MAGIC LEFT JOIN global_temp.ccu013_cohort_definition_old tab2 on tab1.NHS_NUMBER_DEID = tab2.person_id_deid

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM global_temp.ccu013_cohort_definition_old tab1
# MAGIC ANTI JOIN .ccu013_dp_patient_dataset_presence_lookup tab2 on tab1.person_id_deid = tab2.NHS_NUMBER_DEID
...
#         LEFT JOIN .patient_dataset_presence_lookup pres ON pat.NHS_NUMBER_DEID = pres.NHS_NUMBER_DEID
...
# FROM .DP_skinny_patient_01_01_2020_4
