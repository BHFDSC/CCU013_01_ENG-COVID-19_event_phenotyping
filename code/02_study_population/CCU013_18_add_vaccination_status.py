...
# MAGIC %run /Workspaces//CCU013/COVID-19-SEVERITY-PHENOTYPING/CCU013_00_helper_functions
...
[production_date] = spark.sql("SELECT DISTINCT ProductionDate as value from .ccu013_covid_trajectory  LIMIT 1").collect()

# COMMAND ----------

# Get the vaccine data using the production date
vaccine = spark.sql(f'''SELECT PERSON_ID_DEID, DOSE_SEQUENCE, DATE_AND_TIME, ProductionDate FROM .vaccine_status__archive 
...
# MAGIC FROM (SELECT person_id_deid, min(date) as earliest_event FROM .ccu013_covid_trajectory group by person_id_deid) as t
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as dose1 FROM .ccu013_vaccine_status_temp WHERE DOSE_SEQUENCE == 1 GROUP BY person_id_deid) as v 
# MAGIC ON t.person_id_deid = v.person_id_deid
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as dose2 FROM .ccu013_vaccine_status_temp WHERE DOSE_SEQUENCE == 2 GROUP BY person_id_deid) as f
# MAGIC ON t.person_id_deid = f.person_id_deid
# MAGIC INNER JOIN (SELECT person_id_deid, death FROM .ccu013_covid_trajectory_paper_cohort_survival) as d
...
# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_vaccine_status_paper_cohort

# COMMAND ----------

# MAGIC %sql
# MAGIC --- 10/25/21 = obs 1,357,387, ids 1,357,387
# MAGIC SELECT count(*) as observations, count(distinct person_id_deid) as individuals FROM .ccu013_vaccine_status_paper_cohort
...
# MAGIC SELECT count(distinct NHS_NUMBER_DEID) FROM .ccu013_dp_skinny_patient_23_01_2020
...
# MAGIC FROM .ccu013_dp_skinny_patient_23_01_2020 AS a
# MAGIC RIGHT JOIN (SELECT person_id_deid, min(death_date) as death_date FROM .ccu013_tmp_deaths group by person_id_deid) as b
...
# MAGIC SELECT count(distinct a.person_id_deid) FROM (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as a
# MAGIC INNER JOIN (SELECT person_id_deid FROM .ccu013_covid_trajectory_paper_cohort WHERE date < "2020-09-30") as t
...
# MAGIC SELECT a.person_id_deid FROM (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as a
# MAGIC LEFT ANTI JOIN (SELECT person_id_deid FROM .ccu013_covid_trajectory_paper_cohort WHERE date < "2020-09-30") as t
# MAGIC ON a.person_id_deid = t.person_id_deid
# MAGIC LEFT JOIN (SELECT person_id_deid, min(death_date) as death_date FROM .ccu013_tmp_deaths group by person_id_deid) as c
...
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as date FROM .ccu013_covid_trajectory_paper_cohort group by person_id_deid) as b
# MAGIC ON a.person_id_deid = b.person_id_deid
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as dose2 FROM .ccu013_vaccine_status_temp WHERE DOSE_SEQUENCE == 2 GROUP BY person_id_deid) AS c
# MAGIC ON a.person_id_deid = c.person_id_deid
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as dose1 FROM .ccu013_vaccine_status_temp WHERE DOSE_SEQUENCE == 1 GROUP BY person_id_deid) AS d
...
# MAGIC %sql 
# MAGIC SELECT * FROM .ccu013_no_covid_before_wave2_vax_status
...
# MAGIC FROM .ccu013_no_covid_before_wave2_vax_status
...
# MAGIC SELECT count(distinct a.person_id_deid) FROM (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as a
# MAGIC INNER JOIN (SELECT person_id_deid FROM .ccu013_covid_trajectory_paper_cohort WHERE date < "2021-02-01") as t
# MAGIC ON a.person_id_deid = t.person_id_deid
...
# MAGIC SELECT a.person_id_deid FROM (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as a
# MAGIC LEFT ANTI JOIN (SELECT person_id_deid FROM .ccu013_covid_trajectory_paper_cohort WHERE date < "2021-02-01") as t
# MAGIC ON a.person_id_deid = t.person_id_deid
# MAGIC LEFT JOIN (SELECT person_id_deid, min(death_date) as death_date FROM .ccu013_tmp_deaths group by person_id_deid) as c
# MAGIC ON a.person_id_deid = c.person_id_deid
# MAGIC WHERE death_date > "2021-02-01" OR death_date is null 
...
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as dose2 FROM .ccu013_vaccine_status_temp WHERE DOSE_SEQUENCE == 2 GROUP BY person_id_deid) AS b
# MAGIC ON a.person_id_deid = b.person_id_deid
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as dose1 FROM .ccu013_vaccine_status_temp WHERE DOSE_SEQUENCE == 1 GROUP BY person_id_deid) AS c
# MAGIC ON a.person_id_deid = c.person_id_deid)
...
# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_no_covid_before_feb_2021_vax_status
...
# MAGIC SELECT vaccine_status, count(vaccine_status) as count
# MAGIC FROM .ccu013_no_covid_before_feb_2021_vax_status
# MAGIC group by vaccine_status
...
# MAGIC SELECT * FROM .ccu013_no_covid_before_feb_2021_vax_status WHERE (vaccine_status == 'Vaccinated' OR vaccine_status == 'Unvaccinated')) as a
# MAGIC LEFT JOIN (SELECT NHS_NUMBER_DEID as person_id_deid, SEX, ETHNIC, DATE_OF_BIRTH, DATE_OF_DEATH FROM .curr302_patient_skinny_record) as b
# MAGIC ON a.person_id_deid = b.person_id_deid)
...
# MAGIC SELECT vaccine_status, count(vaccine_status) FROM .ccu013_no_covid_before_feb_2021_vax_status_matching group by vaccine_status

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_no_covid_before_feb_2021_vax_status_matching
