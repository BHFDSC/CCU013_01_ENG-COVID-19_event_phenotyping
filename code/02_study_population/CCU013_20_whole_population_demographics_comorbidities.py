...
# MAGIC FROM
# MAGIC   .ccu013_dp_skinny_patient_23_01_2020
...
# MAGIC (SELECT DISTINCT NHS_NUMBER_DEID FROM .ccu013_dp_skinny_patient_23_01_2020) as pop
# MAGIC ANTI JOIN
# MAGIC (SELECT DISTINCT person_id_deid as NHS_NUMBER_DEID FROM .ccu013_covid_events_demographics_paper_cohort) as covid
...
# MAGIC %sql
# MAGIC SELECT COUNT(*), COUNT(Distinct person_id_deid) FROM .ccu013_covid_events_demographics_paper_cohort
...
# MAGIC FROM
# MAGIC (SELECT * FROM
# MAGIC   (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as pop
# MAGIC   ANTI JOIN
# MAGIC   (SELECT person_id_deid FROM .ccu013_covid_events_demographics_paper_cohort) as covid
# MAGIC   ON pop.person_id_deid = covid.person_id_deid
# MAGIC UNION ALL
# MAGIC SELECT person_id_deid FROM .ccu013_covid_events_demographics_paper_cohort
# MAGIC )
...
# MAGIC     FROM .ccu013_dp_skinny_patient_23_01_2020) as pop
# MAGIC     ANTI JOIN
# MAGIC     (SELECT person_id_deid FROM .ccu013_covid_events_demographics_paper_cohort) as covid
# MAGIC     ON pop.person_id_deid = covid.person_id_deid
# MAGIC   UNION ALL
...
# MAGIC     severity 
# MAGIC   FROM 
# MAGIC     .ccu013_covid_events_demographics_paper_cohort
# MAGIC   )
...
def test(table):
  assert table.count() == spark.table(".ccu013_dp_skinny_patient_23_01_2020").count(), "Cohort has diverged in number from descriptive paper skinny table"
...
SELECT * FROM
  ...
  FROM
    (SELECT 
      ...
      0 as death_covid
    FROM .ccu013_dp_skinny_patient_23_01_2020) as pop
    ANTI JOIN
    (SELECT person_id_deid FROM .ccu013_covid_events_demographics_paper_cohort) as covid
...
population_table = ".ccu013_dp_skinny_patient_23_01_2020"

# COVID-19 events
covid_table = ".ccu013_covid_events_demographics_paper_cohort"
# Demographics table
demographics_table = ".ccu013_master_demographics"
# GDPPR (for high risk)
gdppr_table = ".gdppr__archive"
# Comorbidities table
comorbidities_table = ".ccu013_caliber_categories_pre2020"
# Deaths table
deaths_table = ".deaths_"
...
spark.sql(f"OPTIMIZE .{output_table} ZORDER BY person_id_deid")
