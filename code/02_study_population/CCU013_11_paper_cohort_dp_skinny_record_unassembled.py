...
# MAGIC 
# MAGIC **Data output** Table: `.dp_patient_skinny_unassembled`
# MAGIC 
...
spark.sql(f'''
 ...
 SELECT DISTINCT PERSON_ID_DEID as NHS_NUMBER_DEID, 
      ...
      0 as primary,
      FYEAR
      FROM .hes_apc_all_years_archive WHERE ProductionDate == "{production_date}"''')
...
spark.sql(f'''
 CREATE OR REPLACE GLOBAL TEMP VIEW ccu013_dp_all_hes_ae AS
  SELECT DISTINCT PERSON_ID_DEID as NHS_NUMBER_DEID, 
      ...
      0 as primary,
      FYEAR
      FROM .hes_ae_all_years_archive WHERE ProductionDate == "{production_date}"''')
#     FROM .ccu003_direfcts_dataprep_1_ae_all_frzon28may_mm_210528""")
...
spark.sql(f'''
 CREATE OR REPLACE GLOBAL TEMP VIEW ccu013_dp_all_hes_op AS
  SELECT DISTINCT PERSON_ID_DEID as NHS_NUMBER_DEID, 
      ...
      0 as primary,
      FYEAR
      FROM .hes_op_all_years_archive WHERE ProductionDate == "{production_date}"''')
#  FROM .ccu003_direfcts_dataprep_1_op_all_frzon28may_mm_210528"""
...
gdppr_current = spark.sql(f'''
    SELECT * FROM .gdppr__archive as gdppr WHERE ProductionDate == "{production_date}"''')
...
# MAGIC ---                      1 as primary
# MAGIC ---                FROM .ccu003_direfcts_dataprep_1_gdppr_frzon28may_mm_210528 as gdppr 
# MAGIC ---        )
...
deaths = spark.sql(f'''
    SELECT * FROM .deaths__archive WHERE ProductionDate == "{production_date}"''')
...
FROM 
  (SELECT * , row_number() OVER (PARTITION BY DEC_CONF_NHS_NUMBER_CLEAN_DEID 
    ...
    FROM .deaths__archive WHERE ProductionDate == "{production_date}"
    ) cte
WHERE death_rank = 1
...
sgss = spark.sql(f'''
    SELECT * FROM .sgss__archive WHERE ProductionDate == "{production_date}"''')
...
# MAGIC ---FROM (
# MAGIC ---SELECT DISTINCT DEC_CONF_NHS_NUMBER_CLEAN_DEID as NHS_NUMBER_DEID, "deaths" as data_table, 1 as presence FROM .ccu003_direfcts_dataprep_1_deaths_frzon28may_mm_210528
# MAGIC ---union all
# MAGIC ---SELECT DISTINCT PERSON_ID_DEID as NHS_NUMBER_DEID, "sgss" as data_table, 1 as presence FROM .ccu003_direfcts_dataprep_1_sgss_frzon28may_mm_210528
# MAGIC ---union all
...
# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_dp_patient_skinny_unassembled LIMIT 10

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_dp_patient_dataset_presence_lookup LIMIT 10
