...
# MAGIC FROM .ccu013_covid_trajectory
...
# MAGIC SELECT count (DISTINCT person_id_deid) from .ccu013_covid_trajectory
...
# MAGIC   .ccu013_covid_trajectory tab1
# MAGIC INNER JOIN 
# MAGIC   .ccu013_dp_skinny_patient_23_01_2020 tab2 
# MAGIC ON 
...
# MAGIC %sql
# MAGIC OPTIMIZE .ccu013_covid_trajectory_paper_cohort ZORDER BY person_id_deid
...
# MAGIC SELECT count (DISTINCT person_id_deid) from .ccu013_covid_trajectory_paper_cohort
...
# MAGIC SELECT count (*) as total_records from .ccu013_covid_trajectory_paper_cohort

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT covid_phenotype, count (DISTINCT person_id_deid) as unique_ids, count (person_id_deid) as observations
# MAGIC from .ccu013_covid_trajectory_paper_cohort
...
# MAGIC SELECT count (DISTINCT person_id_deid) from .ccu013_covid_severity

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE OR REPLACE GLOBAL TEMP VIEW ccu013_covid_severity_paper_cohort AS
# MAGIC SELECT DISTINCT s.person_id_deid, s.date, s.covid_severity, s.ProductionDate FROM .ccu013_covid_severity as s
# MAGIC INNER JOIN .ccu013_covid_trajectory_paper_cohort as t
...
# MAGIC %sql
# MAGIC REFRESH .ccu013_covid_severity_paper_cohort
...
# MAGIC SELECT count(DISTINCT person_id_deid) FROM .ccu013_covid_severity_paper_cohort

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT count(*) FROM .ccu013_covid_severity_paper_cohort
...
# MAGIC SELECT NHS_NUMBER_DEID AS person_id_deid, DATE('2020-01-23') AS date, '00_Unaffected' AS covid_phenotype, covid_severity, 0 AS phenotype_order, 'Unaffected' AS trajectory_phenotype FROM .ccu013_dp_skinny_patient_23_01_2020 AS a
# MAGIC LEFT JOIN .ccu013_covid_severity_paper_cohort AS b ON a.NHS_NUMBER_DEID = b.person_id_deid)
...
# MAGIC FROM 
# MAGIC .ccu013_covid_trajectory_paper_cohort as tab1
# MAGIC LEFT JOIN .ccu013_covid_severity as tab2 ON tab1.person_id_deid = tab2.person_id_deid
# MAGIC UNION ALL
# MAGIC SELECT person_id_deid, date, covid_phenotype, (CASE WHEN covid_severity IS NULL THEN '00_unaffected' ELSE covid_severity END) AS covid_severity, phenotype_order, trajectory_phenotype FROM (
# MAGIC SELECT NHS_NUMBER_DEID AS person_id_deid, DATE('2020-01-23') AS date, '00_Unaffected' AS covid_phenotype, covid_severity, 0 AS phenotype_order, 'Unaffected' AS trajectory_phenotype FROM .ccu013_dp_skinny_patient_23_01_2020 AS a
# MAGIC LEFT JOIN .ccu013_covid_severity_paper_cohort AS b ON a.NHS_NUMBER_DEID = b.person_id_deid))
...
# MAGIC SELECT * FROM .ccu013_covid_trajectory_graph_data
# MAGIC WHERE person_id_deid = ''

# COMMAND ----------

# MAGIC %sql
# MAGIC REFRESH .ccu013_covid_trajectory_graph_data
...
# MAGIC -- 56609049
# MAGIC -- 57032174
# MAGIC SELECT count (distinct person_id_deid) FROM .ccu013_covid_trajectory_graph_data
...
# MAGIC (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as a
# MAGIC --- Remove anyone with a 
# MAGIC LEFT ANTI JOIN (SELECT person_id_deid FROM .ccu013_covid_trajectory_paper_cohort WHERE date < "2020-03-20") as t
...
# MAGIC FROM .ccu013_covid_trajectory_graph_data as a
# MAGIC INNER JOIN (SELECT j.person_id_deid FROM 
# MAGIC (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as j
...
# MAGIC SELECT count(distinct person_id_deid) FROM .ccu013_covid_trajectory_graph_data_wave1
...
# MAGIC SELECT count(*) FROM .ccu013_covid_trajectory_graph_data_wave1
...
# MAGIC ---SELECT count (distinct person_id_deid) FROM .ccu013_covid_trajectory_paper_cohort_wave1
...
# MAGIC --- 7232055
# MAGIC SELECT count (distinct person_id_deid) FROM .ccu013_covid_trajectory_graph_data_wave1
# MAGIC ---SELECT * FROM .ccu013_covid_trajectory_graph_data_wave1
...
# MAGIC SELECT * FROM 
# MAGIC (SELECT a.person_id_deid, a.date, a.covid_severity, a.trajectory_phenotype, a.phenotype_order, a.days_passed 
# MAGIC FROM .ccu013_covid_trajectory_graph_data_icu as a
# MAGIC INNER JOIN (SELECT j.person_id_deid FROM 
# MAGIC (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as j
# MAGIC --- Remove anyone who had covid before the wave
# MAGIC LEFT ANTI JOIN (SELECT person_id_deid FROM .ccu013_covid_trajectory_paper_cohort WHERE date < "2020-03-20") as t
...
# MAGIC SELECT count(distinct person_id_deid) FROM .ccu013_covid_trajectory_graph_data_wave1_icu
...
# MAGIC -- OLD - 55774208
# MAGIC -- New = 56225024
# MAGIC SELECT count(distinct person_id_deid) FROM .ccu013_covid_trajectory_graph_data_wave2_icu
...
#from pyspark.sql.window import Window
#reinfec = spark.sql(""" 
#SELECT person_id_deid, date FROM .ccu013_covid_trajectory
