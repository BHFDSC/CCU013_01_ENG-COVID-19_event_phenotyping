...
FROM 
  .ccu013_covid_trajectory_paper_cohort
GROUP BY
  person_id_deid
  """)
...
FROM
  .ccu013_covid_events_paper_cohort
""")
...
# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_covid_trajectory_paper_cohort_survival
...
# MAGIC FROM
# MAGIC   .ccu013_covid_trajectory_paper_cohort_survival
# MAGIC GROUP BY 
# MAGIC   severity
...
# MAGIC FROM .ccu013_covid_trajectory_paper_cohort as t
# MAGIC INNER JOIN (SELECT distinct person_id_deid FROM .ccu013_covid_trajectory_paper_cohort WHERE date >= "2020-02-20" AND date <= "2020-05-29" group by person_id_deid) as w
...
SELECT 
  ...
  1 as value
FROM
  .ccu013_covid_trajectory_paper_cohort_wave1
""") \
  .to_koalas() \
...
SELECT
  ...
  ELSE '5_death_only' END as severity
FROM
  .ccu013_covid_events_paper_cohort_wave1
""")
display(severity)
...
INNER JOIN (SELECT DISTINCT person_id_deid, followup_end FROM .ccu013_covid_trajectory_paper_cohort_wave1) as t
...
# MAGIC SELECT DISTINCT t.person_id_deid, date, covid_phenotype
# MAGIC FROM .ccu013_covid_trajectory_paper_cohort as t
# MAGIC INNER JOIN (SELECT distinct person_id_deid FROM .ccu013_covid_trajectory_paper_cohort WHERE date >= "2020-09-30" AND date <= "2021-02-12" group by person_id_deid) as w
# MAGIC ON t.person_id_deid = w.person_id_deid
...
# MAGIC SELECT min (date), max(date), min(followup_end), max(followup_end) FROM .ccu013_covid_trajectory_paper_cohort_wave2
...
SELECT 
  ...
  1 as value
FROM
  .ccu013_covid_trajectory_paper_cohort_wave2
""") \
  .to_koalas() \
  .pivot(index='person_id_deid',
         columns='event', 
...
SELECT
  ...
  ELSE '5_death_only' END as severity
FROM
  .ccu013_covid_events_paper_cohort_wave2
""")
display(severity)
...
INNER JOIN (SELECT DISTINCT person_id_deid, followup_end FROM .ccu013_covid_trajectory_paper_cohort_wave2) as t
ON s.person_id_deid = t.person_id_deid
)  
""")
...
# MAGIC .ccu013_covid_trajectory_paper_cohort as a
# MAGIC LEFT JOIN .ccu013_covid_trajectory_paper_cohort_survival_wave2 as b
...
# MAGIC SELECT COUNT(distinct person_id_deid) FROM .ccu013_covid_trajectory_paper_cohort_wave2

# COMMAND ----------

# MAGIC %sql
# MAGIC --- 9/7/2021 = 2878573
# MAGIC SELECT COUNT(distinct person_id_deid) FROM .ccu013_covid_trajectory_paper_cohort_survival_wave2
...
# MAGIC FROM (SELECT DISTINCT NHS_NUMBER_DEID as person_id_deid FROM .ccu013_dp_skinny_patient_23_01_2020) as a
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as earliest_event FROM .ccu013_covid_trajectory_paper_cohort_wave2 group by person_id_deid) as t
# MAGIC ON a.person_id_deid = t.person_id_deid
# MAGIC LEFT JOIN (SELECT person_id_deid, min(date) as dose2 FROM .ccu013_vaccine_status_temp WHERE DOSE_SEQUENCE == 2 GROUP BY person_id_deid) as v 
# MAGIC ON a.person_id_deid = v.person_id_deid
# MAGIC LEFT JOIN (SELECT person_id_deid, death as covid_death FROM .ccu013_covid_trajectory_paper_cohort_survival_wave2) as d
# MAGIC ON a.person_id_deid = d.person_id_deid
# MAGIC LEFT JOIN (SELECT NHS_NUMBER_DEID as person_id_deid, SEX, ETHNIC, DATE_OF_BIRTH, DATE_OF_DEATH FROM .curr302_patient_skinny_record) as j
...
# MAGIC  SELECT count(distinct person_id_deid) FROM .ccu013_covid_trajectory_paper_cohort_survival_wave2 a
# MAGIC  LEFT ANTI JOIN  .ccu013_vaccine_status_temp as b ON a.person_id_deid = b.person_id_deid
