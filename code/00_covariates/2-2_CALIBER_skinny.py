...
# MAGIC   * GDPPR: `.gdppr_`
# MAGIC   * HES APC: `.hes_apc_all_years`
# MAGIC   
# MAGIC   
# MAGIC **Data output**
...
production_date = spark.sql("SELECT MAX(ProductionDate) FROM .wrang002b_data_version_batchids").first()[0]
print("ProductionDate:", production_date)

# Table names
gdppr_table = ".gdppr__archive" # No non-archive equivalent
hes_apc_table = ".hes_apc_all_years" # Don't need archive as using ProductionDate
...
FROM
   {gdppr_table} as a
INNER JOIN
  .ccu013_caliber_master_codelist as b
ON
  a.CODE = b.code
WHERE
...
# MAGIC %sql
# MAGIC SELECT * FROM .ccu013_caliber_skinny LIMIT 10
