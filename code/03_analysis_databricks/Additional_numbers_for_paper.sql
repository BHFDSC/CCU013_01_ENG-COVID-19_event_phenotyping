...
SELECT COUNT(*) FROM .ccu013_covid_trajectory_paper_cohort
...
FROM .ccu013_covid_events_paper_cohort 
...
SELECT 
  ...
  round(SUM(03_ICU_admission)/(SELECT COUNT(*) FROM .ccu013_covid_events_paper_cohort WHERE 02_Covid_admission = 1)*100,2) as ICU_percent,
  SUM(03_NIV_treatment) as NIV,
  round(SUM(03_NIV_treatment)/(SELECT COUNT(*) FROM .ccu013_covid_events_paper_cohort WHERE 02_Covid_admission = 1)*100,2) as NIV_percent,
  SUM(03_IMV_treatment) as IMV,
  round(SUM(03_IMV_treatment)/(SELECT COUNT(*) FROM .ccu013_covid_events_paper_cohort WHERE 02_Covid_admission = 1)*100,2) as IMV_percent
FROM .ccu013_covid_events_paper_cohort 
...
SELECT 
  ...
  FROM 
    .ccu013_covid_events_demographics_paper_cohort as base
  FULL JOIN
    (SELECT 
      person_id_deid, 
      date, 
...
INNER JOIN
  (SELECT phenotype, category FROM .ccu013_caliber_category_mapping) as mapping
...
FROM 
  .ccu013_dp_skinny_patient_23_01_2020
...
SELECT 
  ...
  ROUND(COUNT(distinct person_id_deid) / (SELECT count(DISTINCT NHS_NUMBER_DEID) FROM .ccu013_dp_skinny_patient_23_01_2020) * 100, 2) as infection_rate
FROM 
...
FROM
  .ccu013_covid_events_demographics_paper_cohort
UNION ALL
SELECT
...
WITH t1 AS 
 (SELECT severity, COUNT(*) AS n 
  FROM .ccu013_covid_events_demographics_paper_cohort
  GROUP BY severity)
SELECT severity, n, 
       n/(SELECT COUNT(*) FROM .ccu013_covid_events_demographics_paper_cohort) * 100 as percentage
FROM t1
ORDER BY
  severity
...
SELECT severity, n, 
       ROUND(n/(SELECT COUNT(*) FROM .ccu013_covid_events_demographics_paper_cohort) * 100,2) as percentage
FROM t1
ORDER BY
  severity
...
SELECT
  ...
  ROUND(SUM(03_NIV_treatment) / 
    (SELECT SUM(03_NIV_treatment) FROM .ccu013_covid_events_demographics_paper_cohort) * 100, 2) as NIV_percent_outICU,
  SUM(CASE WHEN 03_IMV_treatment = 1 then 1 else 0 end) as IMV_n,
  ROUND(SUM(03_IMV_treatment) / 
    (SELECT SUM(03_IMV_treatment) FROM .ccu013_covid_events_demographics_paper_cohort) * 100, 2) as IMV_percent_outICU
FROM
  .ccu013_covid_events_demographics_paper_cohort
WHERE
  03_ICU_admission = 0
...
-- MAGIC %run /Workspaces//CCU013/COVID-19-SEVERITY-PHENOTYPING/03_analysis_databricks/upset_datagen
...
SELECT
  ...
  round(COUNT(*) / 
    (SELECT COUNT(*) FROM .ccu013_covid_events_demographics_paper_cohort WHERE 02_Covid_admission = 1)*100,2) as percentage_hospitalisations
FROM
  .ccu013_01_paper_upset
WHERE
  CHESS = 1
...
SELECT
  ...
  (SELECT COUNT(distinct NHS_NUMBER_DEID) FROM .ccu013_dp_skinny_patient_23_01_2020) as population,
  ROUND(SUM(CASE WHEN date_first >= "2020-09-30" AND date_first <= "2021-02-12" AND 01_Covid_positive_test = 1 then 1 else 0 end)
    /
    (SELECT COUNT(distinct NHS_NUMBER_DEID) FROM .ccu013_dp_skinny_patient_23_01_2020)
