...
WITH t1 AS 
 (SELECT covid_severity, COUNT(*) AS n 
  FROM .ccu013_covid_severity_paper_cohort
  GROUP BY covid_severity)
SELECT covid_severity, n, 
       n/(SELECT COUNT(*) FROM .ccu013_covid_severity_paper_cohort) * 100 as percentage
FROM t1
ORDER BY
  covid_severity
...
FROM
(SELECT NHS_NUMBER_DEID FROM .gdppr__archive
  WHERE CODE = 1300561000000107 AND ProductionDate == "2021-07-29 13:39:04.161949"
    ) as gdppr
INNER JOIN
  (SELECT NHS_NUMBER_DEID FROM .ccu013_dp_skinny_patient_23_01_2020) as denominator
