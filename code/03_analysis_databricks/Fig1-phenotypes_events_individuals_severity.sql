...
SELECT
  ...
  round(COUNT(distinct person_id_deid) / (SELECT COUNT(distinct person_id_deid) FROM .ccu013_covid_events_paper_cohort) *100,2) as percentage
FROM
...
SELECT
  severity,
  COUNT(distinct person_id_deid) as individuals,
  ROUND(COUNT(distinct person_id_deid) / (SELECT COUNT(distinct person_id_deid) FROM .ccu013_covid_events_demographics_paper_cohort) * 100, 2) as percentage
FROM
