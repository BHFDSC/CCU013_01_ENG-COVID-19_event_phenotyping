...
# ---- Load tables ------------------
severity <- dbGetQuery(con, "SELECT * FROM .ccu013_covid_severity")
traject <- dbGetQuery(con, "SELECT * FROM .ccu013_covid_trajectory")
deaths <- dbGetQuery(con, "SELECT * FROM .ccu003_direfcts_dataprep_1_deaths_frzon28may_mm_210528 as d
                     RIGHT JOIN .ccu013_covid_trajectory as t on d.DEC_CONF_NHS_NUMBER_CLEAN_DEID = t.person_id_deid
                     WHERE t.covid_phenotype = '04_Fatal_without_covid_diagnosis'")

severity <- dbGetQuery(con, "SELECT * FROM .ccu013_covid_severity_paper_cohort")
traject <- dbGetQuery(con, "SELECT * FROM .ccu013_covid_trajectory_paper_cohort")
deaths <- dbGetQuery(con, "SELECT * FROM .ccu003_direfcts_dataprep_1_deaths_frzon28may_mm_210528 as d
                     RIGHT JOIN .ccu013_covid_trajectory_paper_cohort as t on d.DEC_CONF_NHS_NUMBER_CLEAN_DEID = t.person_id_deid
                     WHERE t.covid_phenotype = '04_Fatal_without_covid_diagnosis'")
...
myquery <- paste0("select distinct t.person_id_deid, first_event, date, covid_phenotype FROM (
  ...
  (SELECT person_id_deid, min(date) as first_event FROM .ccu013_covid_trajectory_paper_cohort
    group by person_id_deid)
    WHERE first_event > \"", as.character(as.Date(end_date) - 28) ,"\") as f
   INNER JOIN .ccu013_covid_trajectory_paper_cohort as t
