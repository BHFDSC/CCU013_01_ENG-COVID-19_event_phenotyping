...
source("~//CCU013/04_analysis_R-GITHUB-v3/matching_function.R")
...
## -------------------------------------------
d <- dbGetQuery(con, "SELECT * FROM .ccu013_no_covid_before_feb_2021_vax_status_matching")
...
write.table(matched_m[,c("person_id_deid", "id", "vaccinated")], "~//CCU013/output/matching/ccu013_vaccine_matchings.txt", row.names = F, sep = "\t", quote = F)
write.table(matched_summary, "~//CCU013/output/matching/ccu013_vaccine_matchings_summary.txt", row.names = F, sep = "\t", quote = F)
...
# matched_m <- read.table("~//CCU013/output/matching/ccu013_vaccine_matchings.txt", header = T, sep = "\t")
head(matched_m)
...
traject <- dbGetQuery(con, paste0("SELECT a.person_id_deid, date, covid_phenotype, death_date 
                                  ...
                                  .ccu013_tmp_deaths group by person_id_deid) AS b
