...
FROM .ccu013_covid_events_demographics_paper_cohort as a
LEFT JOIN (SELECT person_id_deid, 1 as wave1 FROM .ccu013_covid_trajectory_paper_cohort_survival_wave1) as b
ON a.person_id_deid = b.person_id_deid
LEFT JOIN (SELECT person_id_deid, 1 as wave2 FROM .ccu013_covid_trajectory_paper_cohort_survival_wave2) as c
ON a.person_id_deid = c.person_id_deid)")


cohort <- dbGetQuery(con, "SELECT * FROM .ccu013_covid_events_demographics_paper_cohort")
...
out
write.table(out, "~//CCU013/output/tables/CCU013_supp_tab4_demographic_table_of_deaths.txt", sep = "\t", quote = F)
