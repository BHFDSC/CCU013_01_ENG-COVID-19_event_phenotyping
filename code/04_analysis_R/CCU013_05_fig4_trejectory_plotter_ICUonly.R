...
library(scales)
library(dplyr)
source("~//CCU013/CCU013_colorscheme.R")
source("~//CCU013/trejectory_plot_function.R")

folder_path <- "~//CCU013/output/trajectories/"
...
# ---- Load data -----------------------------------------------
demo <- dbGetQuery(con, "SELECT a.person_id_deid, a.sex, a.ethnic_group, 2021 - left(a.dob,4) as age, a.IMD_quintile FROM .ccu013_master_demographics a 
INNER JOIN (SELECT NHS_NUMBER_DEID FROM .ccu013_dp_skinny_patient_23_01_2020) b
...
FROM .ccu013_covid_trajectory_graph_data_icu as a
INNER JOIN .ccu013_covid_trajectory_paper_cohort as b
...
FROM .ccu013_covid_trajectory_graph_data_icu as a
LEFT ANTI JOIN .ccu013_covid_trajectory_paper_cohort as b
ON a.person_id_deid == b.person_id_deid")
...
FROM .ccu013_covid_trajectory_graph_data_icu")
...
FROM .ccu013_covid_trajectory_graph_data_wave1_icu as a
INNER JOIN .ccu013_covid_trajectory_paper_cohort_wave1 as b
ON a.person_id_deid == b.person_id_deid")
...
FROM .ccu013_covid_trajectory_graph_data_wave1_icu as a
LEFT ANTI JOIN .ccu013_covid_trajectory_paper_cohort_wave1 as b
ON a.person_id_deid == b.person_id_deid")
...
FROM .ccu013_covid_trajectory_graph_data_wave1_icu")
...
FROM .ccu013_covid_trajectory_graph_data_wave2_icu as a
INNER JOIN .ccu013_covid_trajectory_paper_cohort_wave2 as b
ON a.person_id_deid == b.person_id_deid")
...
unaffected <- dbGetQuery(con, "SELECT DISTINCT a.person_id_deid, 'Unaffected' as trajectory, NULL as days 
FROM .ccu013_covid_trajectory_graph_data_wave2_icu as a
LEFT ANTI JOIN .ccu013_covid_trajectory_paper_cohort_wave2 as b
ON a.person_id_deid == b.person_id_deid")
...
FROM .ccu013_covid_trajectory_graph_data_wave2_icu AS a
LEFT JOIN .ccu013_no_covid_before_wave2_vax_status AS b
on a.person_id_deid = b.person_id_deid")
