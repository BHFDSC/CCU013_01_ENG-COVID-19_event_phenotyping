...
# output vars
folder_path <- "~//CCU013/output/km_plots"
...
cohort <- dbGetQuery(con, "SELECT person_id_deid, date_first, death, severity, death_fu_time FROM .ccu013_covid_trajectory_paper_cohort_survival")
nrow(cohort) # 7244925

## Wave population are used to generate wave comparison plots
wave1 <- dbGetQuery(con, "SELECT person_id_deid, date_first, death, severity, death_fu_time FROM .ccu013_covid_trajectory_paper_cohort_survival_wave1")
wave2 <- dbGetQuery(con, "SELECT person_id_deid, date_first, death, severity, death_fu_time FROM .ccu013_covid_trajectory_paper_cohort_survival_wave2")
nrow(wave1) # 264505
nrow(wave2) # 2889028 
wave1[,"wave"] <- 1
wave2[,"wave"] <- 2
...
# Additional data
demo <- dbGetQuery(con, "SELECT * FROM .ccu013_covid_events_demographics_paper_cohort")
nrow(demo)   # 7244925
vax <- dbGetQuery(con, "SELECT * FROM .ccu013_no_covid_before_wave2_vax_status WHERE date is NOT NULL")
nrow(vax)    # 6731281
